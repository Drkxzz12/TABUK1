...
class TouristRecommendationService {
  ...
  static Future<TouristPreferences?> getUserPreferences() async {
    try {
      ...
      if (kDebugMode) print('${AppConstants.errorLoadingTouristPreferences}: $e');
      return null;
    }
  }
  ...
  static Future<List<Hotspot>> getPersonalizedRecommendations({int limit = 10}) async {
    try {
      ...
      if (preferences == null) {
        throw RecommendationException('No preferences found', RecommendationErrorType.noPreferences);
      }
      ...
      throw RecommendationException('Network error', RecommendationErrorType.networkError);
    }
  }

  /// Score and sort hotspots based on preferences
  static List<Hotspot> _scoreAndSortHotspots(List<Hotspot> hotspots, TouristPreferences preferences) {
    final scored = hotspots.map((hotspot) {
  ...
  static double _calculateScore(Hotspot hotspot, TouristPreferences preferences) {
    return _weights['base']! +
           _scoreByKeywords(hotspot, preferences.destinationTypes, 'destinationType') * _weights['destinationType']! +
           _scoreByKeywords(hotspot, [preferences.vibe], 'vibe') * _weights['vibe']! +
           _scoreByKeywords(hotspot, [preferences.companion], 'companion') * _weights['companion']! +
           ...
           _scoreLesserKnown(hotspot, preferences.lesserKnown) * _weights['lesserKnown']! +
           _scoreEvent(hotspot, preferences.eventRecommendation) * _weights['event']!;
  }

  /// Generic keyword scoring
  static double _scoreByKeywords(Hotspot hotspot, List<String> selections, String mappingKey) {
    double score = 0.0;
    for (final selection in selections) {
      final keywords = _keywordMappings[mappingKey]?[selection] ?? [];
      score += keywords.where((keyword) => _containsKeyword(hotspot, keyword)).length.toDouble();
    }
    return score;
  }

  /// Companion-specific scoring
  static double _scoreCompanionSpecific(Hotspot hotspot, TouristPreferences preferences) {
    switch (preferences.companion) {
      case 'With Family':
        return (hotspot.restroom && hotspot.foodAccess ? 0.5 : 0.0) +
               (hotspot.entranceFee != null && hotspot.entranceFee! <= 100 ? 0.3 : 0.0);
      case 'Solo':
      ...
      case 'With Partner':
        return (hotspot.category.toLowerCase().contains('scenic') || 
                hotspot.category.toLowerCase().contains('romantic')) ? 0.5 : 0.0;
      default:
        return 0.0;
    }
  }
  ...
  static double _scoreTiming(Hotspot hotspot, String timing) {
    switch (timing) {
      case 'Off-Season (Less crowded)':
        return ['hidden', 'secret'].any((keyword) => _containsKeyword(hotspot, keyword)) ? 1.0 : 0.0;
      case 'Festival Seasons':
        return ['festival', 'event', 'cultural'].any((keyword) => _containsKeyword(hotspot, keyword)) ? 1.0 : 0.0;
      default:
        return 0.2;
    }
  }
  ...
  static double _scoreLesserKnown(Hotspot hotspot, String preference) {
    switch (preference) {
      case 'Yes, I love discovering hidden gems':
        return ['hidden', 'secret', 'undiscovered', 'local'].any((keyword) => _containsKeyword(hotspot, keyword)) ? 1.5 : 0.0;
      case 'No, I prefer popular and established places':
        final popularBonus = ['popular', 'famous', 'well-known'].any((keyword) => _containsKeyword(hotspot, keyword)) ? 1.0 : 0.0;
        final hiddenPenalty = ['hidden', 'secret'].any((keyword) => _containsKeyword(hotspot, keyword)) ? -0.5 : 0.0;
        return popularBonus + hiddenPenalty;
      case 'Only if they are easy to access':
        return (_containsKeyword(hotspot, 'accessible') || _containsKeyword(hotspot, 'easy') || hotspot.transportation.isNotEmpty) ? 0.8 : 0.0;
      default:
        return 0.0;
    }
  }
  ...
  static double _scoreEvent(Hotspot hotspot, String eventPreference) {
    return (eventPreference == 'Yes' && ['event', 'festival', 'cultural'].any((keyword) => _containsKeyword(hotspot, keyword))) ? 1.0 : 0.0;
  }
  ...
  static Future<List<Hotspot>> searchHotspots(
    ...
    try {
      ...
      if (query.trim().isEmpty && _hasFilters(districts, municipalities, categories)) {
        return await _searchWithFirestore(limit, districts, municipalities, categories);
      }
      
      // Otherwise use cached search
      return await _searchWithCache(query, limit, districts, municipalities, categories, preferences);
    } catch (e) {
  ...
  static bool _hasFilters(List<String>? districts, List<String>? municipalities, List<String>? categories) {
    return (districts?.isNotEmpty ?? false) || (municipalities?.isNotEmpty ?? false) || (categories?.isNotEmpty ?? false);
  }

  static Future<List<Hotspot>> _searchWithFirestore(
    int limit, List<String>? districts, List<String>? municipalities, List<String>? categories) async {
    var query = FirebaseFirestore.instance.collection(AppConstants.hotspotsCollection) as Query<Map<String, dynamic>>;
    
    if (categories?.isNotEmpty == true) query = query.where('category', whereIn: categories!.take(10).toList());
    if (districts?.isNotEmpty == true) query = query.where('district', whereIn: districts!.take(10).toList());
    if (municipalities?.isNotEmpty == true) query = query.where('municipality', whereIn: municipalities!.take(10).toList());
  ...
  static Future<List<Hotspot>> _searchWithCache(
    String query, int limit, List<String>? districts, List<String>? municipalities, 
    List<String>? categories, TouristPreferences? preferences) async {
  ...
  static bool _matchesSearch(Hotspot hotspot, String query) {
    return query.isEmpty || [hotspot.name, hotspot.description, hotspot.category]
  ...
  static Future<List<Hotspot>> getJustForYouRecommendations({int limit = 10}) async {
...
static Future<List<Hotspot>> getNearbyRecommendations({
  ...
  try {
    ...
    final validHotspots = hotspots.where((h) => 
        h.isArchived != true && 
        h.latitude != null && 
        h.longitude != null &&
...
static bool _isValidCoordinate(double lat, double lng) {
  ...
  static Future<List<Hotspot>> getHiddenGemsRecommendations({int limit = 10}) async {
    final recommendations = await getPersonalizedRecommendations(limit: 50);
    return recommendations.where((h) => _isHiddenGem(h) && h.isArchived != true).take(limit).toList();
  }

  static bool _isHiddenGem(Hotspot hotspot) {
    return ['hidden', 'secret', 'undiscovered', 'local'].any((keyword) => 
  ...
  static Future<List<Hotspot>> getTrendingHotspots({int limit = 10}) async {
    ...
    trending.sort((a, b) => a.name.compareTo(b.name)); // Replace with actual trending logic
...
/// Exception handling
enum RecommendationErrorType { networkError, noPreferences, noData, authError, unknown }
...
class HotspotCache {
  ...
  static Future<List<Hotspot>> getCachedHotspots() async {
    if (_cachedHotspots == null || _lastFetch == null || 
  ...
  static Future<void> _refreshCache() async {
    final snapshot = await FirebaseFirestore.instance
        ...
        .get();
    _cachedHotspots = snapshot.docs.map((doc) => Hotspot.fromMap(doc.data(), doc.id)).toList();
